...
function showListings(array) {
  ...
  <div class="w-full p-6 rounded-md bg-white shadow-lg text-sm font-light flex flex-col gap-4 font-quickS">
    <div class="flex flex-col justify-between gap-6">
      ...
      <div class="flex flex-row overflow-scroll gap-4 h-80">${media}</div>
    </div>
    <hr>
    <div class="flex flex-col gap-4 pt-4">
      ...
      <div class="pt-4">
        ...
        <p>Current bid:</p>
          <div class="flex flex-row gap-4 items-baseline">${bids}</div>
        </div>
      </div>
    </div>
  </div>
  ...
  for (let lis of array) {
    ...
    if (lis.bids !== "") {
      ...
      for (let bid of lis.bids) {
        ...
        oneBid = `<div class="flex flex-row justify-between items-center gap-2 text-gray-400 last-of-type:bg-white last-of-type:text-black last-of-type:outline-2 last-of-type:outline last-of-type:outline-blue last-of-type:p-2 last-of-type:px-3">
                  <p>${bidderName}</p>
                  <p>${bid["amount"]}</p>
              </div>
              `;
  ...
  for (let lis of array) {
    ...
    <div class="w-full p-6 rounded-md bg-white shadow-lg text-sm font-light flex flex-col gap-4 font-quickS">
      ...
      <div class="flex flex-col gap-4 pt-4">
        ...
        <div class="pt-4">
          <p>${bidNumber}</p>
          <div class="my-4 w-full flex flex-row justify-between items-baseline border-b border-blue md:w-2/3 m-auto px-4">          <p>Current bid:</p>
            <div class="flex flex-row gap-4 items-baseline">${bids}</div>
          </div>
        </div>
      </div>
...
function editListing(id, title, desc, tags, media) {
  ...
  <form
  class="p-6 mt-12 md:shadow-none flex flex-col w-full gap-8 md:gap-8 font-quickS font-light lg:justify-center lg:items-center">
  <div
    class="flex flex-col w-full gap-4 lg:w-2/3 lg:justify-between lg:gap-8">
    <div>
      <input
        type="text"
        name="title"
